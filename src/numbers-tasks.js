...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const dotX = Math.abs(x2 - x1);
  const dotY = Math.abs(y2 - y1);
  const distSquare = dotX ** 2 + dotY ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const square1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const square2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const mult = square1 * square2;
  const cos = sum / mult;
